...
import com.baseframe.comm.constant.RedisErrorEnumBase;
...
public class RedisUtil {
    ...
    public void expire(final String key, final int time)
        ...
        if (ObjectUtils.isEmpty(key) || time < 0) {
            ...
            throw new RedisException(RedisErrorEnumBase.REDIS_PARAM_EXCEPTION);
        }
        try {
            redisTemplate.expire(key, time, TimeUnit.SECONDS);
        } catch (Exception e) {
            log.error("REDIS数据失败，连接异常，请检查相关配置。错误：{}", e.getMessage());
            throw new RedisException(RedisErrorEnumBase.REDIS_CONN_EXCEPTION);
        }
    }
